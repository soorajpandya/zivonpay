    ...
    <section className="relative min-h-screen overflow-x-hidden pt-24" style={{ background: "var(--gradient-hero)" }}>
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-24 lg:py-32">
        <div className="flex flex-col items-center text-center">
          ...
          >
            <div className="rounded-xl border border-border/50 bg-card p-2 shadow-2xl" style={{ boxShadow: "var(--shadow-card)" }}>
              <div className="rounded-lg bg-secondary/50 p-6">
                {/* Mock dashboard header */}
                <div className="mb-6 flex items-center justify-between">
                  <div>
                    <p className="text-sm text-muted-foreground">Today's Collection</p>
                    <p className="text-3xl font-bold text-foreground" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>₹12,45,890</p>
                  </div>
                  <div className="flex gap-2">
                    <div className="rounded-md bg-primary/10 px-3 py-1.5 text-xs font-medium text-primary">+23.5%</div>
                  </div>
                </div>
                {/* Mock chart bars */}
                <div className="flex items-end gap-2">
                  {[40, 65, 45, 80, 55, 90, 70, 85, 60, 95, 75, 88].map((h, i) => (
                    <motion.div
                      ...
                      style={{
                        background: `linear-gradient(to top, hsl(200 90% 50% / 0.1), hsl(200 90% 50% / ${h / 100 * 0.4}))`,
                      }}
                    />
                  ))}
                </div>
